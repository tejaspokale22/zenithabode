    ...
    return (
        <div className="sticky inset-x-0 top-0 z-30 bg-opacity-60 backdrop-blur-lg bg-zinc-100">
            <div className="container flex justify-between items-center px-4 py-4 sm:px-6 md:px-8">
                ...
                <button className="block z-40 ml-2 lg:hidden" onClick={toggleMenu} aria-label="Toggle menu">
                    {isMenuOpen ? (
            ...
            <div className={`lg:hidden transition-all duration-300 ease-in-out overflow-hidden bg-white shadow-md ${isMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'} w-full`}> 
                <nav className="flex flex-col gap-y-4 items-center px-3 py-6 sm:px-6">
                    {nav.map(({ href, label }) => (
                        <Link
                            key={href}
                            href={href}
                            className={`text-base sm:text-lg w-full text-center ${pathname === href ? 'text-black font-bold' : 'text-gray-600 hover:text-black'}`}
